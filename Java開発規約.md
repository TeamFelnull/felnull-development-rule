...
処理がない場合でも`default`は必ず記述してください。

### finalizeメソッド*

`finalize`メソッドはオーバーライドしないでください。

### コードブロック*

括弧がない`if`,`while`,`for`を書かないでください。
...
文字列を比較する際は`=`ではなく、`equals`を使用してください。
...
- setter、getter以外、@Override,@Testアノテーションが付いていないのメソッド
- フィールド
